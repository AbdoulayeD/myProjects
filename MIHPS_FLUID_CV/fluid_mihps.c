...
#define LS(my_i, my_j) x[build_index(my_i,my_j, grid_size)] = (a * ( x[build_index(my_i-1, my_j, grid_size)] + x[build_index(my_i+1, my_j, grid_size)] +   x[build_index(my_i, my_j-1, grid_size)] + x[build_index(my_i, my_j+1, grid_size)]) +  x0[build_index(my_i, my_j, grid_size)]) * toto;
...
void linearSolver(int b, float* x, float* x0, float a, float c, float dt, int grid_size)
  {
  int i,j,k;
  const float toto=1.0f/c;
  for (k = 0; k < 20; k++)
    {
    for (j = 1; j <= grid_size; j+=3)
      {
